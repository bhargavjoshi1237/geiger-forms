...
> **This product's schema is `forms`.** Its migration ledger is
> `forms.geiger_migrations`. Substitute `forms` wherever this doc writes
...
(`forms.tickets`), never `forms.flow_tickets`.
...
-- Add seat maps
--
-- Owns forms.seat_maps. Free-form header: what this changes and why.

-- @up
create table if not exists forms.seat_maps (
...
-- @down
drop table if exists forms.seat_maps;
```
...
create table if not exists forms.tickets (...);
alter table forms.tickets add column if not exists tier text;   -- back-fill older copies
create index if not exists tickets_event_idx on forms.tickets (event_id);

drop trigger if exists tickets_touch_updated_at on forms.tickets;
create trigger tickets_touch_updated_at
before update on forms.tickets
for each row execute function forms.touch_updated_at();

drop policy if exists tickets_demo_all on forms.tickets;
create policy tickets_demo_all on forms.tickets for all to anon, authenticated
...
```sql
alter table forms.tickets enable row level security;
drop policy if exists tickets_demo_all on forms.tickets;
create policy tickets_demo_all on forms.tickets for all to anon, authenticated
  using (true) with check (true);
```
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on forms.tickets (event_id);
```
