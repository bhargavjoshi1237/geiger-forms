...
export function FormsScreenShell({ eyebrow, title, description, action, actionHref, stats = [], children }) {
  return (
    <div className="mx-auto w-full space-y-8 px-2 py-4 text-foreground lg:max-w-[85%] lg:px-0">
      <div className="flex flex-col gap-4 border-b border-border pb-6 md:flex-row md:items-center md:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 max-w-2xl text-sm font-medium text-muted-foreground">{description}</p>
        </div>
        ...
        <div className="flex shrink-0 flex-col gap-4 md:items-end">
          {stats.length ? (
            <div className="flex gap-6">
              {stats.map((stat) => (
                <div key={stat.label} className="flex flex-col gap-1 md:items-end">
                  <span className="text-[10px] font-medium uppercase tracking-wider text-text-secondary">{stat.label}</span>
                  <span className="text-base font-medium w-full text-center text-foreground">{stat.value}</span>
                </div>
              ))}
            </div>
          ) : null}
          ...
          {action && actionHref ? (
            <Button asChild type="button" className="w-fit shrink-0 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
              <Link href={actionHref}>{action}</Link>
            </Button>
          ) : action ? (
            <Button type="button" className="w-fit shrink-0 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
              {action}
            </Button>
          ) : null}
        </div>
