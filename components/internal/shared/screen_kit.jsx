 ...
 * suite's events area so every workspace screen reads as one product (dark
 * surfaces, semantic tokens, muted text). This project's Badge has no `variant`,
 * so StatusPill colors via a className map instead.
 */

// --- Page header -------------------------------------------------------------

// Plain title + description over a bottom divider, primary action on the right.
...
// Header for an in-editor section that opts out of the detail editor's default
// title block (`ownHeader: true`). Title + description left, action(s) right.
...
// Renders a formatted value with each digit animated as a rolling odometer;
// prefixes/separators like "$" and "," stay static.
export function RollingNumber({ value, className }) {
  ...
  // Precompute each character's stagger delay (nth-digit * 70ms; null for
  // non-digits) before the JSX so nothing is reassigned inside a render closure.
  const delays = [];
  let digitIndex = 0;
  for (const char of chars) {
    if (/\d/.test(char)) {
      delays.push(digitIndex * 70);
      digitIndex += 1;
    } else {
      delays.push(null);
    }
    ...
    <span className={cn("inline-flex tabular-nums", className)}>
      {chars.map((char, i) =>
        delays[i] === null ? (
          ...
          <RollingDigit key={i} digit={Number(char)} active={active} delay={delays[i]} />
        ),
      )}
    </span>
  );
...
/**
 * Single-card KPI row with divider-separated cells and animated values.
 * Each stat: { label, value, delta?, trend?, footer? }.
 */
...
// map[status] → { label?, dotClass?, className?, dot? }. Colors come from the
// className (this project's Badge has no variant prop).
...
/**
 * A labelled settings row. Pass `control` for a custom control or
 * `checked`/`onCheckedChange` to render a Switch automatically.
 */
